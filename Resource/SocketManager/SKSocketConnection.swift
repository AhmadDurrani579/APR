...
class SKSocketConnection: NSObject {
    ...
    public let socket = SocketManager(socketURL: URL(string: "http://162.0.237.13:3000")!, config: [.log(true), .compress])
...
//            self.sockets.emit("channel")
        ...
        sockets.onAny { (socketEvent) in
            ...
            print("print socket Event %@", socketEvent.event)
            ...
            if socketEvent.event == "allchat" {
                let message = socketEvent.items![0]
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "AllChatList"), object : message, userInfo : nil)
            }
        }
...
//               self.sockets.emit("users"  , "187" , "148")
    ...
    func getChatList(obj : [String : Any]) -> Void {
        self.getSocketCurrentState { (isConnected, status ) in
            if isConnected {
//               self.sockets.emit("users"  , "187" , "148")
                self.sockets.emit("getchat" , obj)
            }else {
                print("Not connected")
            }
        }
